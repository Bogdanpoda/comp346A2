...
public class Server extends Thread {
    ...
    private static String serverThreadRunningStatus1="terminated";	 /* Running status of thread 1 - idle, running, terminated */
    private static String serverThreadRunningStatus2="terminated";	 /* Running status of thread 2 - idle, running, terminated */
    ...
    {   int accIndex;             	/* Index position of account to update */
        ...
        if(serverThreadId.equals("thread1"))
            serverThreadRunningStatus1="processing";

        if(serverThreadId.equals("thread2"))
            serverThreadRunningStatus2="processing";
        ...
        while ((!Network.getClientConnectionStatus().equals("disconnected")))
        {
            ...
            if (!Network.getInBufferStatus().equals("empty"))
           {
               ...
               if(serverThreadId.equals("thread1"))
                   serverThreadRunningStatus1="before trans in";

               if(serverThreadId.equals("thread2"))
                   serverThreadRunningStatus2="before trans in";
                Network.transferIn(trans);                              /* Transfer a transaction from the network input buffer */
                    ...
                    if (trans.getOperationType().equals("DEPOSIT")) {
                        ...
                        //System.out.println("\nAccount number: " + trans.getAccountNumber() + " Balance: " + trans.getTransactionBalance() + " Message " + trans.getTransactionStatus());
               ...
               if(serverThreadId.equals("thread1"))
                   serverThreadRunningStatus1="before trans out";

               if(serverThreadId.equals("thread2"))
                   serverThreadRunningStatus2="before trans out";

                Network.transferOut(trans);
               if(serverThreadId.equals("thread1"))
                   serverThreadRunningStatus1="after trans out";

               if(serverThreadId.equals("thread2"))
                   serverThreadRunningStatus2="after trans out";

                setNumberOfTransactions( (getNumberOfTransactions() +  1) ); 	/* Count the number of transactions processed */
            }

            if(serverThreadId.equals("thread1"))
                serverThreadRunningStatus1="after trans out and while";

            if(serverThreadId.equals("thread2"))
                serverThreadRunningStatus2="after trans out and while";
...
//                }
       ...
       // System.out.println("\n server id: "+serverThreadId+"\n server 1: "+serverThreadRunningStatus1+"\n server 2 "+serverThreadRunningStatus2);
